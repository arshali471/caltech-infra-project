...
locals {
  ...
  # ---- 003 — fifth connector: multi-table routed, no schema-change topic ------
  # Same router+unwrap shape as 001 (specific table list → one topic), plus
  # include.schema.changes = false so no separate schema-change topic is emitted.
  oracle_003_config = merge(
    local.oracle_debezium_iam_auth,
    local.oracle_debezium_pdb,
    local.oracle_converters,
    {
      ...
      "database.dbname"             = var.oracle_db_name
      "database.connection.adapter" = var.oracle_connection_adapter
      "topic.prefix"                = var.oracle_003_topic_prefix
      "schema.include.list"         = var.oracle_003_schema_include_list
      "table.include.list"          = var.oracle_003_table_include_list
      "snapshot.mode"               = var.oracle_snapshot_mode
      "heartbeat.interval.ms"       = tostring(var.debezium_heartbeat_interval_ms)
      "decimal.handling.mode"       = "double"
      "time.precision.mode"         = "connect"
      ...
      "max.batch.size"              = "20000"
      "poll.interval.ms"            = "100"
      "include.schema.changes"      = tostring(var.oracle_003_include_schema_changes)

      "transforms"                               = "router,unwrap"
      "transforms.router.type"                   = "io.debezium.transforms.ByLogicalTableRouter"
      "transforms.router.topic.regex"            = ".*"
      "transforms.router.topic.replacement"      = var.oracle_003_router_topic
      "transforms.router.key.enforce.uniqueness" = "true"
      "transforms.unwrap.type"                   = "io.debezium.transforms.ExtractNewRecordState"
      "transforms.unwrap.drop.tombstones"        = "false"
      "transforms.unwrap.delete.handling.mode"   = "drop"
      ...
      "schema.history.internal.kafka.bootstrap.servers"          = module.msk.bootstrap_brokers_iam
      "schema.history.internal.kafka.topic"                      = var.oracle_003_history_topic
      "schema.history.internal.store.only.captured.tables.ddl"   = tostring(var.oracle_store_only_captured_tables_ddl)
      "schema.history.internal.store.only.captured.database.ddl" = tostring(var.oracle_store_only_captured_database_ddl)
    }
  )

  # ---- 004 — sixth connector: schema_only snapshot, scoped router regex -------
  # Differs from the other routed connectors: snapshot.mode = schema_only (no
  # data backfill, only schema then live changes), a SPECIFIC router regex that
  # only rewrites topics under its own prefix, and it OMITS heartbeat.interval.ms,
  # schema.include.list, and table.include.list (captures everything LogMiner
  # sees, filtered downstream by the router regex).
  oracle_004_config = merge(
    local.oracle_debezium_iam_auth,
    local.oracle_debezium_pdb,
    local.oracle_converters,
    {
      ...
      "database.dbname"             = var.oracle_db_name
      "database.connection.adapter" = var.oracle_connection_adapter
      "topic.prefix"                = var.oracle_004_topic_prefix
      "snapshot.mode"               = var.oracle_004_snapshot_mode
      "decimal.handling.mode"       = "double"
      "time.precision.mode"         = "connect"
      "max.queue.size"              = "200000"
      "max.batch.size"              = "20000"
      ...
      "transforms"                               = "router,unwrap"
      "transforms.router.type"                   = "io.debezium.transforms.ByLogicalTableRouter"
      "transforms.router.topic.regex"            = var.oracle_004_router_topic_regex
      "transforms.router.topic.replacement"      = var.oracle_004_router_topic
      "transforms.router.key.enforce.uniqueness" = "true"
      "transforms.unwrap.type"                   = "io.debezium.transforms.ExtractNewRecordState"
      "transforms.unwrap.drop.tombstones"        = "false"
      "transforms.unwrap.delete.handling.mode"   = "drop"
      ...
      "schema.history.internal.kafka.bootstrap.servers"          = module.msk.bootstrap_brokers_iam
      "schema.history.internal.kafka.topic"                      = var.oracle_004_history_topic
      "schema.history.internal.store.only.captured.tables.ddl"   = tostring(var.oracle_store_only_captured_tables_ddl)
      "schema.history.internal.store.only.captured.database.ddl" = tostring(var.oracle_store_only_captured_database_ddl)
    }
  )

  # ---- 005 — seventh connector: unwrap only, schema_only, no router ----------
  # Like 004 (schema_only, distinct prefix, no schema/table include) but with
  # NO router SMT — only unwrap — and heartbeat.interval.ms present. Its own
  # history topic (schemahistory.oracle005) keeps it isolated.
  oracle_005_config = merge(
    local.oracle_debezium_iam_auth,
    local.oracle_debezium_pdb,
    local.oracle_converters,
    {
      ...
      "database.dbname"             = var.oracle_db_name
      "database.connection.adapter" = var.oracle_connection_adapter
      "topic.prefix"                = var.oracle_005_topic_prefix
      "snapshot.mode"               = var.oracle_005_snapshot_mode
      "heartbeat.interval.ms"       = tostring(var.debezium_heartbeat_interval_ms)
      "decimal.handling.mode"       = "double"
      "time.precision.mode"         = "connect"
      "max.queue.size"              = "200000"
      ...
      "schema.history.internal.kafka.bootstrap.servers"          = module.msk.bootstrap_brokers_iam
      "schema.history.internal.kafka.topic"                      = var.oracle_005_history_topic
      "schema.history.internal.store.only.captured.tables.ddl"   = tostring(var.oracle_store_only_captured_tables_ddl)
      "schema.history.internal.store.only.captured.database.ddl" = tostring(var.oracle_store_only_captured_database_ddl)
    }
  )

  # ---- 006 — eighth connector: unwrap only, regex table filter, LogMiner tune -
  # Like 005 (unwrap only, schema_only) but restricts to EXETER.SSS_* tables via
  # a regex table.include.list, and adds LogMiner tuning properties supplied via
  # var.oracle_006_extra_config (a raw map so the exact keys/values live in
  # tfvars and can be corrected without touching this file). The extra map is
  # merged LAST, so anything in it overrides the keys below.
  oracle_006_config = merge(
    local.oracle_debezium_iam_auth,
    local.oracle_debezium_pdb,
    local.oracle_converters,
    {
      ...
      "database.dbname"             = var.oracle_db_name
      "database.connection.adapter" = var.oracle_connection_adapter
      "topic.prefix"                = var.oracle_006_topic_prefix
      "schema.include.list"         = var.oracle_006_schema_include_list
      "table.include.list"          = var.oracle_006_table_include_list
      "snapshot.mode"               = var.oracle_006_snapshot_mode
      "heartbeat.interval.ms"       = tostring(var.debezium_heartbeat_interval_ms)
      "decimal.handling.mode"       = "double"
      "time.precision.mode"         = "connect"
      "max.queue.size"              = "200000"
      "max.batch.size"              = "20000"
      "poll.interval.ms"            = "100"
      "include.schema.changes"      = tostring(var.oracle_006_include_schema_changes)
      ...
      "schema.history.internal.kafka.bootstrap.servers"          = module.msk.bootstrap_brokers_iam
      "schema.history.internal.kafka.topic"                      = var.oracle_006_history_topic
      "schema.history.internal.store.only.captured.tables.ddl"   = tostring(var.oracle_store_only_captured_tables_ddl)
      "schema.history.internal.store.only.captured.database.ddl" = tostring(var.oracle_store_only_captured_database_ddl)
    },
  ...
  # ---- schema-alltables — router capture of ALL tables in a schema -----------
  # Router folds every table matched by table.include.list (regex EXETER\..*)
  # into a single topic. Distinct topic.prefix / router topic / history topic so
  # it never collides with the other connectors.
  oracle_alltables_config = merge(
    local.oracle_debezium_iam_auth,
    local.oracle_debezium_pdb,
    local.oracle_converters,
    {
      ...
      "database.dbname"             = var.oracle_db_name
      "database.connection.adapter" = var.oracle_connection_adapter
      "topic.prefix"                = var.oracle_alltables_topic_prefix
      "schema.include.list"         = var.oracle_alltables_schema_include_list
      "table.include.list"          = var.oracle_alltables_table_include_list
      "snapshot.mode"               = var.oracle_alltables_snapshot_mode
      "heartbeat.interval.ms"       = tostring(var.debezium_heartbeat_interval_ms)
      "decimal.handling.mode"       = "double"
      "time.precision.mode"         = "connect"
      "max.queue.size"              = "200000"
      ...
      "transforms.router.type"                   = "io.debezium.transforms.ByLogicalTableRouter"
      "transforms.router.topic.regex"            = ".*"
      "transforms.router.topic.replacement"      = var.oracle_alltables_router_topic
      "transforms.router.key.enforce.uniqueness" = "true"
      "transforms.unwrap.type"                   = "io.debezium.transforms.ExtractNewRecordState"
      "transforms.unwrap.drop.tombstones"        = "false"
      "transforms.unwrap.delete.handling.mode"   = "drop"
      ...
      "schema.history.internal.kafka.bootstrap.servers"          = module.msk.bootstrap_brokers_iam
      "schema.history.internal.kafka.topic"                      = var.oracle_alltables_history_topic
      "schema.history.internal.store.only.captured.tables.ddl"   = tostring(var.oracle_store_only_captured_tables_ddl)
      "schema.history.internal.store.only.captured.database.ddl" = tostring(var.oracle_store_only_captured_database_ddl)
    }
  )
...
###############################################################################
# Step 9f — Fifth Oracle connector: 003 (multi-table routed, no schema changes)
# Independent module, created alongside connectors 1–4 without replacing them.
# See local.oracle_003_config.
###############################################################################

module "msk_connect_oracle_003" {
  source = "./modules/msk_connect"

  count = var.enable_oracle_003_connector ? 1 : 0

  name                  = local.name
  connector_name_suffix = var.oracle_003_name_suffix
  custom_plugin_name    = var.oracle_connect_custom_plugin_name
  bootstrap_servers     = module.msk.bootstrap_brokers_iam
  msk_connect_sg_id     = module.security_groups.msk_connect_sg_id
  private_subnet_ids    = local.msk_connect_subnet_ids
  ...
  connector_configuration = local.oracle_003_config

  tags = var.tags
}

###############################################################################
# Step 9g — Sixth Oracle connector: 004 (schema_only, scoped router regex)
# Independent module, created alongside connectors 1–5 without replacing them.
# See local.oracle_004_config.
###############################################################################

module "msk_connect_oracle_004" {
  source = "./modules/msk_connect"

  count = var.enable_oracle_004_connector ? 1 : 0

  name                  = local.name
  connector_name_suffix = var.oracle_004_name_suffix
  custom_plugin_name    = var.oracle_connect_custom_plugin_name
  bootstrap_servers     = module.msk.bootstrap_brokers_iam
  msk_connect_sg_id     = module.security_groups.msk_connect_sg_id
  private_subnet_ids    = local.msk_connect_subnet_ids
  ...
  connector_configuration = local.oracle_004_config

  tags = var.tags
}

###############################################################################
# Step 9h — Seventh Oracle connector: 005 (unwrap only, schema_only)
# Independent module, created alongside connectors 1–6 without replacing them.
# See local.oracle_005_config.
###############################################################################

module "msk_connect_oracle_005" {
  source = "./modules/msk_connect"

  count = var.enable_oracle_005_connector ? 1 : 0

  name                  = local.name
  connector_name_suffix = var.oracle_005_name_suffix
  custom_plugin_name    = var.oracle_connect_custom_plugin_name
  bootstrap_servers     = module.msk.bootstrap_brokers_iam
  msk_connect_sg_id     = module.security_groups.msk_connect_sg_id
  private_subnet_ids    = local.msk_connect_subnet_ids
  ...
  connector_configuration = local.oracle_005_config

  tags = var.tags
}

###############################################################################
# Step 9i — Eighth Oracle connector: 006 (regex table filter, LogMiner tuning)
# Independent module, created alongside connectors 1–7 without replacing them.
# See local.oracle_006_config.
###############################################################################

module "msk_connect_oracle_006" {
  source = "./modules/msk_connect"

  count = var.enable_oracle_006_connector ? 1 : 0

  name                  = local.name
  connector_name_suffix = var.oracle_006_name_suffix
  custom_plugin_name    = var.oracle_connect_custom_plugin_name
  bootstrap_servers     = module.msk.bootstrap_brokers_iam
  msk_connect_sg_id     = module.security_groups.msk_connect_sg_id
  private_subnet_ids    = local.msk_connect_subnet_ids
...
###############################################################################
# Step 9k — Oracle connector: schema-alltables (router capture of all tables)
# Independent module. See local.oracle_alltables_config.
###############################################################################

module "msk_connect_oracle_alltables" {
  source = "./modules/msk_connect"

  count = var.enable_oracle_alltables_connector ? 1 : 0

  name                  = local.name
  connector_name_suffix = var.oracle_alltables_name_suffix
  custom_plugin_name    = var.oracle_connect_custom_plugin_name
  bootstrap_servers     = module.msk.bootstrap_brokers_iam
  msk_connect_sg_id     = module.security_groups.msk_connect_sg_id
  private_subnet_ids    = local.msk_connect_subnet_ids
  ...
  connector_configuration = local.oracle_alltables_config
